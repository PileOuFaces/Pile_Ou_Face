...
# VS Code display languages (vscode.env.language); anything else falls back to English.
...
REPORT_STRINGS: dict[str, dict[str, str]] = {
    "fr": {
        "title": "Rapport d'auto-triage IA",
        "disclaimer": (
        ...
        "no_category": "Aucune catégorie détectée.",
        "priority_functions": "Fonctions prioritaires",
        "col_address": "Adresse",
        "col_original_name": "Nom original",
        "col_suggested_name": "Nom suggéré",
        "col_status": "Statut",
        "status_error": "erreur",
        "status_ok": "ok",
        "detail_by_function": "Détail par fonction",
        "error_prefix": "Erreur",
        "no_description": "_(pas de description)_",
        "statistics": "Statistiques",
        "processed": "Fonctions traitées",
        "annotated": "Fonctions annotées",
        "duration": "Durée",
    ...
    "en": {
        ...
        "disclaimer": (
            "> Automatically generated by AI. All annotations are reversible "
            "and never overwrite a manual annotation — verify before use."
        ),
        "model_used": "Model used",
        "exec_summary": "Executive Summary",
        "classification": "Classification",
        "no_category": "No category detected.",
        "priority_functions": "Priority Functions",
        "col_address": "Address",
        "col_original_name": "Original Name",
        "col_suggested_name": "Suggested Name",
        "col_status": "Status",
        "status_error": "error",
        "status_ok": "ok",
        "detail_by_function": "Function Details",
        "error_prefix": "Error",
        "no_description": "_(no description)_",
        "statistics": "Statistics",
        "processed": "Functions processed",
        "annotated": "Functions annotated",
        "duration": "Duration",
...
def normalize_language(raw: str | None) -> str:
    """Map a VS Code locale (e.g. "fr", "pt-br", "zh-CN") to a supported short code.

    Validated against LANGUAGE_NAMES (broad set) rather than REPORT_STRINGS
    (fr/en only), so LLM-generated content still follows the app's language
    for locales whose static report template isn't translated yet — those
    fall back to the English template via _report_strings.
    """
    code = str(raw or "en").strip().lower().split("-")[0]
    return code if code in LANGUAGE_NAMES else "en"


def _report_strings(language: str) -> dict[str, str]:
    return REPORT_STRINGS.get(language, REPORT_STRINGS["en"])
...
def synthesize_binary_summary(
    ...
    facts = "\n".join(
        f"- {a.name} -> {a.generated_name or '?'}: {a.docstring or '(undocumented)'}"
        for a in analyses
        if not a.error
    )
    if not facts:
    ...
    language_name = LANGUAGE_NAMES.get(language, "English")
    prompt = (
        "Here are the automatically annotated functions of a binary, with their "
        f"dominant detected categories: {', '.join(categories) or 'none'}.\n"
        f"Write a 3-5 sentence executive summary in {language_name}, without inventing "
...
def render_markdown_report(
    ...
    s = _report_strings(language)
    lines = [
        f"# {s['title']} — {Path(binary_path).name}",
        "",
        s["disclaimer"],
        "",
        f"{s['model_used']} : `{provider}{f'@{model}' if model else ''}`",
        "",
        f"## {s['exec_summary']}",
        "",
        summary.get("text", ""),
        ...
        f"## {s['classification']}",
        "",
        ", ".join(summary.get("categories", [])) or s["no_category"],
        "",
        f"## {s['priority_functions']}",
        "",
        f"| {s['col_address']} | {s['col_original_name']} | {s['col_suggested_name']} | {s['col_status']} |",
        "|---|---|---|---|",
    ]
    for analysis in analyses:
        status = s["status_error"] if analysis.error else s["status_ok"]
        lines.append(
            f"| {analysis.addr} | {analysis.name} | {analysis.generated_name or '-'} | {status} |"
        )

    lines += ["", f"## {s['detail_by_function']}", ""]
    for analysis in analyses:
        lines.append(f"### {analysis.name} ({analysis.addr})")
        if analysis.error:
            lines.append(f"_{s['error_prefix']}: {analysis.error}_")
        else:
            lines.append(analysis.docstring or s["no_description"])
        lines.append("")
