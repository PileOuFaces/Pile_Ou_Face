...
class TestAnalyzeFunction(unittest.TestCase):
    ...
    @patch("backends.mcp.auto_triage._call_tool")
    @patch("backends.mcp.auto_triage.call_provider_result")
    def test_prompt_requests_docstring_in_requested_language(
        self, mock_call, mock_tool
    ):
        mock_tool.return_value = {"ok": False}
        mock_call.return_value = {
            "text": '{"name": "fn2", "docstring": "...", "tags": []}'
        }
        at.analyze_function(
            self._candidate(), "/tmp/bin", "ollama", None, at.TriageBudget(), "de"
        )
...
class TestNormalizeLanguage(unittest.TestCase):
    def test_maps_vscode_locale_variants_to_base_code(self):
        self.assertEqual(at.normalize_language("fr"), "fr")
        self.assertEqual(at.normalize_language("fr-FR"), "fr")
        self.assertEqual(
            at.normalize_language("pt-br"), "pt"
        )  # known LANGUAGE_NAMES entry, base code kept
...
class TestRenderMarkdownReport(unittest.TestCase):
    def test_report_contains_expected_sections(self):
        ...
        summary = {"text": "Dummy summary.", "categories": ["filesystem"]}
        stats = {"processed": 2, "annotated": 1, "elapsed_s": 1.234}
        report = at.render_markdown_report("/tmp/some.bin", analyses, summary, stats)
        self.assertIn("# AI Auto-triage Report", report)
        self.assertIn("Executive Summary", report)
        self.assertIn("Classification", report)
        self.assertIn("Priority Functions", report)
        self.assertIn("parse_header", report)
        self.assertIn("fn_b", report)
        self.assertIn("Automatically generated by AI", report)

    def test_report_follows_requested_language(self):
        analyses = [
            at.FunctionAnalysis(
                addr="0x1000",
                name="fn_a",
                generated_name="parse_header",
                docstring="Analyse un en-tete.",
                tags=["filesystem"],
            ),
        ]
        summary = {"text": "Resume bidon.", "categories": ["filesystem"]}
        stats = {"processed": 1, "annotated": 1, "elapsed_s": 1.234}
        report = at.render_markdown_report(
            "/tmp/some.bin", analyses, summary, stats, language="fr"
        )
...
class TestRunAutoTriageEndToEnd(unittest.TestCase):
    ...
    def test_end_to_end_annotates_functions_with_ai_source(self):
        ...
        try:
            ...
            self.assertIn("Priority Functions", result["report_markdown"])
