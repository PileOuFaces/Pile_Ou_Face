...
CFG_CALLGRAPH_UNFIXTURED_ADAPTERS: set[str] = set()
...
class TestFunctionArchSupportMatrix(unittest.TestCase):
    ...
    def test_wasm_is_disasm_only_until_module_level_targets_are_modeled(self):
        matrix = get_feature_support_matrix()
        support = matrix["wasm"]

        self.assertEqual(support["disasm"]["level"], "full")
        for feature in FUNCTION_FEATURES:
