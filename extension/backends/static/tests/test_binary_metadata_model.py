...
def build_mocked_model(binary=None, dwarf=None, dwarf_error=None, plt=None):
    with (
        patch(
            "backends.static.binary.binary_metadata_model.Path.exists",
            return_value=True,
        ),
        patch(
            "backends.static.binary.binary_metadata_model.load_binary_facts",
            return_value=binary or FAKE_BINARY,
        ),
        patch(
            "backends.static.binary.binary_metadata_model.extract_sections",
            return_value=FAKE_SECTIONS,
        ),
        patch(
            "backends.static.binary.binary_metadata_model.section_flags_by_name",
            return_value={".text": ["READ", "EXEC"], ".data": ["READ", "WRITE"]},
        ),
        patch(
            "backends.static.binary.binary_metadata_model.load_symbols",
            return_value=FAKE_SYMBOLS,
        ),
        patch(
            "backends.static.binary.binary_metadata_model.load_dwarf_functions",
            return_value=(dwarf or [], dwarf_error),
        ),
        patch(
            "backends.static.binary.binary_metadata_model.load_plt_entries",
            return_value=plt
            or [
                {
                    "name": "printf",
...
class TestBinaryMetadataModel(unittest.TestCase):
    ...
    def test_pie_vs_non_pie_runtime_fields(self):
        ...
        self.assertEqual(
            model["runtime"], {"base": "0x0", "entry": "0x1050", "pie": True}
        )
    ...
    def test_stripped_binary_still_has_stable_shape(self):
        ...
        with (
            ...
            patch(
                "backends.static.binary.binary_metadata_model.load_dwarf_functions",
                return_value=([], "Aucune info DWARF"),
            ),
            patch(
                "backends.static.binary.binary_metadata_model.load_plt_entries",
                return_value=[],
    ...
    def test_real_fixture_elf_if_toolchain_available(self):
        ...
        self.assertTrue(
            any(section["name"] == ".text" for section in model["sections"])
        )
