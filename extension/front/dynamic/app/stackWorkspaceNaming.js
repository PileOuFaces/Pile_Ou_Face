...
export function finalizeDisplayEntry(entry, { functionName, bpRegister, registerArguments, allEntries, allObservations } = {}) {
  ...
  return {
    ...
    pointerKind: clean(entry?.primaryObservation?.pointerKind),
    pointerSubKind: clean(entry?.primaryObservation?.pointerSubKind),
    pointerTarget: entry?.primaryObservation?.pointerTarget ?? null,
